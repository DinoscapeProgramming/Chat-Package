...
function readFile() {
  return new Promise((resolve, reject) => {
    fs.readFile(module.exports.options.file, function (err, data) {
      ...
      return resolve({ action: "readFile", result: data });
    });
  });
}
...
function readFileSync() {
  try {
    return { action: "readFileSync", result: JSON.parse(fs.readFileSync(module.exports.options.file, 'utf8')) };
  } catch (err) {
    return { action: "readFileSync", err: err.message };
  }
}

function isURL(options, extraOptions) {
  return new Promise((resolve, reject) => {
...
function isURLSync(options, extraOptions) {
...
function createRoom(userId, options, extraOptions) {
  return new Promise((resolve, reject) => {
    ...
    if (!Object.keys(readFileSync().result.users).includes(userId)) return resolve({ action: "createRoom", err: "Invalid user id" });
      ...
      crypto.randomBytes(10, function (err, token) {
        ...
        crypto.randomBytes(4, function (err, id) {
          ...
          fs.writeFile(module.exports.options.file, JSON.stringify({
            users: readFileSync().result.users,
            rooms: {
              ...readFileSync().result.rooms,
              ...{
                [id.toString('hex')]: {
                  name: options.name,
                  privat: false,
                  users: {
                    [userId]: {
                      username: readFileSync().result.users[userId].username
                    }
                  },
                  messages: []
                }
...
function createRoomSync(userId, options, extraOptions) {
  ...
  if (!Object.keys(readFileSync().result.users).includes(userId)) return { action: "createRoomSync", err: "Invalid user id" };
...
function editRoom(userId, options, extraOptions) {
  return new Promise((resolve, reject) => {
    ...
    if (!Object.keys(readFileSync().result.rooms).includes(options.id)) return resolve({ action: "editRoom", err: "Id does not exist" });
    if (!userId) return resolve({ action: "editRoom", err: "No user id was given" });
    if (!Object.keys(readFileSync().result.users).includes(userId)) return resolve({ action: "editRoom", err: "Invalid user id" });
    if (Object.keys(readFileSync().result.rooms[options.id].users)[0] !== userId) return resolve({ action: "editRoom", err: "You are not the creator of this room" });
    var buffer = readFileSync().result.rooms;
    ...
    fs.writeFile(module.exports.options.file, JSON.stringify({ users: readFileSync().result.users, rooms: buffer }), 'utf8', function (err) {
...
function editRoomSync(userId, options, extraOptions) {
  ...
  if (!Object.keys(readFileSync().result.rooms).includes(options.id)) return { action: "editRoomSync", err: "Id does not exist" };
  if (!userId) return { action: "editRoomSync", err: "No user id was given" };
  if (!Object.keys(readFileSync().result.users).includes(userId)) return { action: "editRoomSync", err: "Invalid user id" };
  if (Object.keys(readFileSync().result.rooms[options.id].users)[0] !== userId) return { action: "editRoomSync", err: "You are not the creator of this room" };
  var buffer = readFileSync().result.rooms;
  if (options.options.name) {
  ...
  try {
  fs.writeFileSync(module.exports.options.file, JSON.stringify({ users: readFileSync().result.users, rooms: buffer }), 'utf8');
  } catch (err) {
...
function deleteRoom(userId, options, extraOptions) {
  return new Promise((resolve, reject) => {
    ...
    if (!Object.keys(readFileSync().result.rooms).includes(options.id)) return resolve({ action: "deleteRoom", err: "Id does not exist" });
    if (!userId) return resolve({ action: "deleteRoom", err: "No user id was given" });
    if (!Object.keys(readFileSync().result.users).includes(userId)) return resolve({ action: "deleteRoom", err: "Invalid user id" });
    if (Object.keys(readFileSync().result.rooms[options.id].users)[0] !== userId) return resolve({ action: "deleteRoom", err: "You are not the creator of this room" });
    var buffer = readFileSync().result.rooms;
...
function deleteRoomSync(userId, options, extraOptions) {
  ...
  if (!Object.keys(readFileSync().result.rooms).includes(options.id)) return { action: "deleteRoomSync", err: "Id does not exist" };
  if (!userId) return { action: "deleteRoomSync", err: "No user id was given" };
  if (!Object.keys(readFileSync().result.users).includes(userId)) return { action: "deleteRoomSync", err: "Invalid user id" };
  if (Object.keys(readFileSync().result.rooms[options.id].users)[0] !== userId) return { action: "deleteRoomSync", err: "You are not the creator of this room" };
  var buffer = readFileSync().result.rooms;
  delete buffer[options.id];
  try {
    fs.writeFileSync(module.exports.options.file, JSON.stringify({ users: readFileSync().result.users, rooms: buffer }), 'utf8');
...
function getRoom(userId, options, extraOptions) {
  return new Promise((resolve, reject) => {
    ...
    if (!Object.keys(readFileSync().result.rooms).includes(options.id)) return resolve({ action: "getRoom", err: "Id does not exist" });
    var data = {};
    if (options.request.includes("name")) {
      data.name = readFileSync().result.rooms[options.id].name;
    }
    if (options.request.includes("icon")) {
      data.icon = readFileSync().result.rooms[options.id].icon;
    }
    if (options.request.includes("messages")) {
      if (readFileSync().result.rooms[options.id].privat === true && (!Object.keys(options).includes("token") || !options.token)) return resolve({ action: "getRoom", err: "No token was given" });
      if (readFileSync().result.rooms[options.id].privat === true && (readFileSync().result.rooms[options.id].token !== options.token)) return resolve({ action: "getRoom", err: "Invalid token" });
      data.messages = readFileSync().result.rooms[options.id].messages;
    }
...
function getRoomSync(userId, options, extraOptions) {
  ...
  if (!Object.keys(readFileSync().result.rooms).includes(options.id)) return { action: "getRoomSync", err: "Id does not exist" };
  var data = {};
  if (options.request.includes("name")) {
    data.name = readFileSync().result.rooms[options.id].name;
  }
  ...
  if (options.request.includes("messages")) {
    if (readFileSync().result.rooms[options.id].privat === true && (!Object.keys(options).includes("token") || !options.token)) return { action: "getRoom", err: "No token was given" };
    if (readFileSync().result.rooms[options.id].privat === true && (readFileSync().result.rooms[options.id].token !== options.token)) return { action: "getRoom", err: "Invalid token" };
    data.messages = readFileSync().result.rooms[options.id].messages;
  }
...
function allRooms(userId, options, extraOptions) {
  return new Promise((resolve, reject) => {
    ...
    if (!Object.keys(readFileSync().result.users).includes(userId)) return resolve({ action: "allRooms", err: "Invalid user id" });
    if (Object.keys(options).includes("items") && options.items && (typeof options.item === "number")) {
      return resolve({ action: "allRooms", userId, rooms: Object.entries(readFileSync().result.rooms).filter((item) => item[1].privat === false).map((item) => ({ id: item[0], name: item[1].name })).slice(0, options.items) });
    } else {
      return resolve({ action: "allRooms", userId, rooms: Object.entries(readFileSync().result.rooms).filter((item) => item[1].privat === false).map((item) => ({ id: item[0], name: item[1].name })) });
    }
  });
}
...
function allRoomsSync(userId, options, extraOptions) {
  ...
  if (!Object.keys(readFileSync().result.users).includes(userId)) return resolve({ action: "allRoomsSync", err: "Invalid user id" });
  if (Object.keys(options).includes("items") && options.items && (typeof options.item === "number")) {
    return { action: "allRoomsSync", userId, rooms: Object.entries(readFileSync().result.rooms).filter((item) => item[1].privat === false).map((item) => ({ id: item[0], name: item[1].name })).slice(0, options.items) };
  } else {
    return { action: "allRoomsSync", userId, rooms: Object.entries(readFileSync().result.rooms).filter((item) => item[1].privat === false).map((item) => ({ id: item[0], name: item[1].name })) };
  }
}
...
function joinRoom(userId, options, extraOptions) {
  return new Promise((resolve, reject) => {
    ...
    if (!Object.keys(readFileSync().result.rooms).includes(options.id)) return resolve({ action: "joinRoom", err: "Id does not exist" });
    if (!userId) return resolve({ action: "joinRoom", err: "No user id was given" });
    if (!Object.keys(readFileSync().result.users).includes(userId)) return resolve({ action: "joinRoom", err: "Invalid user id" });
    if (readFileSync().result.rooms[options.id].privat === true && (!Object.keys(options).includes("token") || !options.token)) return resolve({ action: "joinRoom", err: "No token was given" });
    if (readFileSync().result.rooms[options.id].privat === true && (readFileSync().result.rooms[options.id].token !== options.token)) return resolve({ action: "joinRoom", err: "Invalid token" });
    var buffer = readFileSync().result.rooms;
    ...
    fs.writeFile(module.exports.options.file, JSON.stringify({ users: readFileSync().result.users, rooms: buffer }), 'utf8', function (err) {
      ...
      return resolve({ actiom: "joinRoom", userId, id: options.id, username: readFileSync().result.users[userId].username });
    });
  });
}
...
function joinRoomSync(userId, options, extraOptions) {
  ...
  if (!Object.keys(readFileSync().result.rooms).includes(options.id)) return { action: "joinRoomSync", err: "Id does not exist" };
  if (!userId) return { action: "joinRoomSync", err: "No user id was given" };
  if (!Object.keys(readFileSync().result.users).includes(userId)) return { action: "joinRoomSync", err: "Invalid user id" };
  if (readFileSync().result.rooms[options.id].privat === true && (!Object.keys(options).includes("token") || !options.token)) return { action: "joinRoomSync", err: "No token was given" };
  if (readFileSync().result.rooms[options.id].privat === true && (readFileSync().result.rooms[options.id].token !== options.token)) return { action: "joinRoomSync", err: "Invalid token" };
  var buffer = readFileSync().result.rooms;
  buffer[options.id].users[userId] = {
    username: readFileSync().result.users[userId].username
  };
  ...
  return { actiom: "joinRoomSync", userId, id: options.id, username: readFileSync().result.users[userId].username };
}
...
function leaveRoom(userId, options, extraOptions) {
  return new Promise((resolve, reject) => {
    ...
    if (!Object.keys(readFileSync().result.rooms).includes(options.id)) return resolve({ action: "leaveRoom", err: "Id does not exist" });
    if (!userId) return resolve({ action: "leaveRoom", err: "No user id was given" });
    if (!Object.keys(readFileSync().result.users).includes(userId)) return resolve({ action: "leaveRoom", err: "Invalid user id" });
    if (readFileSync().result.rooms[options.id].privat === true && (!Object.keys(options).includes("token") || !options.token)) return resolve({ action: "leaveRoom", err: "No token was given" });
    if (readFileSync().result.rooms[options.id].privat === true && (readFileSync().result.rooms[options.id].token !== options.token)) return resolve({ action: "leaveRoom", err: "Invalid token" });
    var buffer = readFileSync().result.rooms;
    delete buffer[options.id].users[userId];
    fs.writeFile(module.exports.options.file, JSON.stringify({ users: readFileSync().result.users, rooms: buffer }), 'utf8', function (err) {
      if (err) return resolve({ action: "leaveRoom", err: err.message });
      return resolve({ action: "leaveRoom", userId, id: options.id, username: readFileSync().result.users[userId].username })
    });
  });
}
...
function leaveRoomSync(userId, options, extraOptions) {
  ...
  if (!Object.keys(readFileSync().result.rooms).includes(options.id)) return { action: "leaveRoomSync", err: "Id does not exist" };
  if (!userId) return { action: "leaveRoomSync", err: "No user id was given" };
  if (!Object.keys(readFileSync().result.users).includes(userId)) return { action: "leaveRoomSync", err: "Invalid user id" };
  if (readFileSync().result.rooms[options.id].privat === true && (!Object.keys(options).includes("token") || !options.token)) return { action: "leaveRoomSync", err: "No token was given" };
  if (readFileSync().result.rooms[options.id].privat === true && (readFileSync().result.rooms[options.id].token !== options.token)) return { action: "leaveRoomSync", err: "Invalid token" };
  var buffer = readFileSync().result.rooms;
  delete buffer[options.id].users[userId];
  try {
    fs.writeFileSync(module.exports.options.file, JSON.stringify({ users: readFileSync().result.users, rooms: buffer }), 'utf8');
  ...
  return { action: "leaveRoomSync", userId, id: options.id, username: readFileSync().result.users[userId].username }
}
...
function sendMessage(userId, options, extraOptions) {
  return new Promise((resolve, reject) => {
    ...
    if (!Object.keys(readFileSync().result.rooms[options.id].users).includes(userId)) return resolve({ action: "sendMessage", err: "Invalid user id" });
    ...
    var buffer = readFileSync().result.rooms;
    buffer[options.id].messages.push([options.message, { userId, username: readFileSync().result.users[userId].username }]);
    fs.writeFile(module.exports.options.file, JSON.stringify({ users: readFileSync().result.users, rooms: buffer }), 'utf8', function (err) {
      if (err) return resolve({ action: "sendMessage", err: err.message });
      return resolve({ action: "sendMessage", userId, username: readFileSync().result.users[userId].username, id: options.id, message: options.message, messageId: readFileSync().result.rooms[options.id].messages.length });
    });
  });
}
...
function sendMessageSync(userId, options, extraOptions) {
  ...
  if (!Object.keys(readFileSync().result.rooms[options.id].users).includes(userId)) return { action: "sendMessageSync", err: "Invalid user id" };
  ...
  return { action: "sendMessageSync", userId, username: readFileSync().result.users[userId].username, id: options.id, message: options.message, messageId: readFileSync().result.rooms[options.id].messages.length };
}
...
function editMessage(userId, options, extraOptions) {
  return new Promise((resolve, reject) => {
    ...
    if (!Object.keys(readFileSync().result.rooms[options.id].users).includes(userId)) return resolve({ action: "editMessage", err: "Invalid user id" });
    if (!Object.keys(options).includes("id") || !options.id) return resolve({ action: "editMessage", err: "No id was given" });
    if (!Object.keys(readFileSync().result.rooms).includes(options.id)) return resolve({ action: "editMessage", err: "Invalid id" });
    ...
    if ((typeof options.messageId !== "number") || options.messageId < 1 || options.messageId > readFileSync().result.rooms[options.id].messages.length) return resolve({ action: "editMessage", err: "Invalid message id" });
    if (readFileSync().result.rooms[options.id].messages[messageId - 1][1].userId !== userId) return resolve({ action: "editMessage", err: "You have not send this message" });
    ...
    var buffer = readFileSync().result.rooms;
    buffer[options.id].messages[messageId - 1] = [options.options.message || readFileSync().result.rooms[options.id].messages[messageId - 1][0], { userId, username: readFileSync().result.users[userId].username }];
    fs.writeFile(module.exports.options.file, JSON.stringify({ users: readFileSync().result.users, rooms: buffer }), 'utf8', function (err) {
      if (err) return resolve({ action: "editMessage", err: err.message });
      return resolve({ action: "editMessage", userId, username: readFileSync().result.users[userId].username, id: options.id, options: options.options, messageId: options.messageId });
    });
  });
}
...
function editMessageSync(userId, options, extraOptions) {
  ...
  if (!Object.keys(readFileSync().result.rooms[options.id].users).includes(userId)) return { action: "editMessageSync", err: "Invalid user id" };
  if (!Object.keys(options).includes("id") || !options.id) return { action: "editMessageSync", err: "No id was given" };
  if (!Object.keys(readFileSync().result.rooms).includes(options.id)) return { action: "editMessageSync", err: "Invalid id" };
  if (!Object.keys(options).includes("messageId") || !options.messageId) return { action: "editMessageSync", err: "No message id was given" };
  if ((typeof options.messageId !== "number") || options.messageId < 1 || options.messageId > readFileSync().result.rooms[options.id].messages.length) return { action: "editMessageSync", err: "Invalid message id" };
  if (readFileSync().result.rooms[options.id].messages[messageId - 1][1].userId !== userId) return { action: "editMessageSync", err: "You have not send this message" };
  ...
  return { action: "editMessageSync", userId, username: readFileSync().result.users[userId].username, id: options.id, options: options.options, messageId: options.messageId };
}
...
function deleteMessage(userId, options, extraOptions) {
  return new Promise((resolve, reject) => {
    ...
    if (!Object.keys(readFileSync().result.rooms).includes(options.id)) return resolve({ action: "deleteMessage", err: "Invalid id" });
    if (!userId) return resolve({ action: "deleteMessage", err: "No user id was given" });
    if (!Object.keys(readFileSync().result.rooms[options.id].users).includes(userId)) return resolve({ action: "deleteMessage", err: "Invalid user id" });
    ...
    if ((typeof options.messageId !== "number") || options.messageId < 1 || options.messageId > readFileSync().result.rooms[options.id].messages.length) return resolve({ action: "deleteMessage", err: "Invalid message id" });
    if (readFileSync().result.rooms[options.id].messages[messageId - 1][1].userId !== userId) return resolve({ action: "deleteMessage", err: "You have not send this message" });
    var buffer = readFileSync().result.rooms;
    buffer[options.id].messages.splice(options.messageId - 1, 1);
    fs.writeFile(module.exports.options.file, JSON.stringify({ users: readFileSync().result.users, rooms: buffer }), 'utf8', function (err) {
      if (err) return resolve({ action: "deleteMessage", err: err.message });
      return resolve({ action: "deleteMessage", userId, username: readFileSync().result.users[userId].username, id: options.id, messageId: options.messageId });
    });
  });
}
...
function deleteMessageSync(userId, options, extraOptions) {
  ...
  if (!Object.keys(readFileSync().result.rooms).includes(options.id)) return { action: "deleteMessageSync", err: "Invalid id" };
  if (!userId) return { action: "deleteMessageSync", err: "No user id was given" };
  if (!Object.keys(readFileSync().result.rooms[options.id].users).includes(userId)) return { action: "deleteMessageSync", err: "Invalid user id" };
  if (!Object.keys(options).includes("messageId") || !options.messageId) return { action: "deleteMessageSync", err: "No message id was given" };
  if ((typeof options.messageId !== "number") || options.messageId < 1 || options.messageId > readFileSync().result.rooms[options.id].messages.length) return resolve({ action: "deleteMessageSync", err: "Invalid message id" });
  if (readFileSync().result.rooms[options.id].messages[messageId - 1][1].userId !== userId) return resolve({ action: "deleteMessageSync", err: "You have not send this message" });
  var buffer = readFileSync().result.rooms;
  buffer[options.id].messages.splice(options.messageId - 1, 1);
  try {
    fs.writeFileSync(module.exports.options.file, JSON.stringify({ users: readFileSync().result.users, rooms: buffer }), 'utf8');
  ...
  return { action: "deleteMessageSync", userId, username: readFileSync().result.users[userId].username, id: options.id, messageId: options.messageId };
}
...
function getMessage(userId, options, extraOptions) {
  return new Promise((resolve, reject) => {
    ...
    if (!Object.keys(readFileSync().result.rooms).includes(options.id)) return resolve({ action: "getMessage", err: "Invalid id" });
    if (!userId) return resolve({ action: "getMessage", err: "No user id was given" });
    if (!Object.keys(readFileSync().result.rooms[options.id].users).includes(userId)) return resolve({ action: "getMessage", err: "Invalid user id" });
    if (!Object.keys(options).includes("messageId") || !options.messageId) return resolve({ action: "getMessage", err: "No message id was given" });
    if ((typeof options.messageId !== "number") || options.messageId < 1 || options.messageId > readFileSync().result.rooms[options.id].messages.length) return resolve({ action: "getMessage", err: "Invalid message id" });
    if (!Object.keys(readFileSync().result.rooms[options.id].users).includes(userId)) return resolve({ action: "getMessage", err: "You are not in the room where the message was send" });
    return resolve({ action: "getMessage", id: options.id, messageId: options.messageId, result: { message: readFileSync().result.rooms[options.id].messages[options.messageId - 1][0], messageAuthor: { userId: readFileSync().result.rooms[options.id].messages[options.messageId - 1][1].userId, username: readFileSync().result.rooms[options.id].messages[options.messageId - 1][1].username } } });
  });
}
...
function getMessageSync(userId, options, extraOptions) {
  ...
  if (!Object.keys(readFileSync().result.rooms).includes(options.id)) return { action: "getMessage", err: "Invalid id" };
  if (!userId) return { action: "getMessage", err: "No user id was given" };
  if (!Object.keys(readFileSync().result.rooms[options.id].users).includes(userId)) return { action: "getMessage", err: "Invalid user id" };
  if (!Object.keys(options).includes("messageId") || !options.messageId) return { action: "getMessage", err: "No message id was given" };
  if ((typeof options.messageId !== "number") || options.messageId < 1 || options.messageId > readFileSync().result.rooms[options.id].messages.length) return { action: "getMessage", err: "Invalid message id" };
  if (!Object.keys(readFileSync().result.rooms[options.id].users).includes(userId)) return { action: "getMessage", err: "You are not in the room where the message was send" };
  return { action: "getMessage", id: options.id, messageId: options.messageId, result: { message: readFileSync().result.rooms[options.id].messages[options.messageId - 1][0], messageAuthor: { userId: readFileSync().result.rooms[options.id].messages[options.messageId - 1][1].userId, username: readFileSync().result.rooms[options.id].messages[options.messageId - 1][1].username } } };
}
...
function createUser(userId, options, extraOptions) {
  return new Promise((resolve, reject) => {
    ...
    fs.writeFile(module.exports.options.file, JSON.stringify({
      ...
      rooms: readFileSync().result.rooms
    }), 'utf8', function (err) {
...
function editUser(userId, options, extraOptions) {
  return new Promise((resolve, reject) => {
    ...
    if (!Object.keys(readFileSync().result.users).includes(userId)) return resolve({ action: "editUser", err: "Invalid user id" });
    ...
    fs.writeFile(module.exports.options.file, JSON.stringify({
      users: {
        ...
        ...{
          [userId]: {
            username: options?.options?.username || readFileSync().result.users[userId].username,
            avatar: options?.options?.avatar || readFileSync().result.users[userId]?.avatar || null
          }
        }
      },
      rooms: readFileSync().result.rooms
...
function editUserSync(userId, options, extraOptions) {
  ...
  if (!Object.keys(readFileSync().result.users).includes(userId)) return { action: "editUserSync", err: "Invalid user id" };
...
function deleteUser(userId, options, extraOptions) {
  return new Promise((resolve, reject) => {
    ...
    if (!Object.keys(readFileSync().result.users).includes(userId)) return resolve({ action: "deleteUser", err: "Invalid user id" });
    var username = readFileSync().result.users[userId].username;
    var buffer = readFileSync().result.users;
    delete buffer[userId];
    Object.entries(readFileSync().result.rooms).forEach((room) => {
      leaveRoom(room[1], { userId });
    });
    fs.writeFile(module.exports.options.file, JSON.stringify({ users: buffer, rooms: readFileSync().result.rooms }), 'utf8', function (err) {
...
function deleteUserSync(userId, options, extraOptions) {
  ...
  if (!Object.keys(readFileSync().result.users).includes(userId)) return { action: "deleteUserSync", err: "Invalid user id" };
  var user = readFileSync().result.users[userId];
  var buffer = readFileSync().result.users;
  delete buffer[userId];
  Object.entries(readFileSync().result.rooms).forEach((room) => {
    leaveRoom(room[1], { userId });
  ...
  try {
    fs.writeFileSync(module.exports.options.file, JSON.stringify({ users: buffer, rooms: readFileSync().result.rooms }), 'utf8');
  } catch (err) {
...
function getUser(userId, options, extraOptions) {
  return new Promise((resolve, reject) => {
    ...
    if (!Object.keys(readFileSync().result.users).includes(userId)) return resolve({ action: "getUser", err: "Invalid user id" });
    return resolve({ action: "getUser", userId, username: readFileSync().result.users[userId].username, avatar: readFileSync().result.users?.[userId].avatar || null });
  });
}

function getUserSync(userId, options, extraOptions) {
  if (!userId) return { action: "getUserSync", err: "No user id was given" };
  if (!Object.keys(readFileSync().result.users).includes(userId)) return { action: "getUserSync", err: "Invalid user id" };
  return { action: "getUserSync", userId, username: readFileSync().result.users[userId].username, avatar: readFileSync().result.users?.[userId].avatar || null };
}
